...
const initialData: ClientePlan[] = [
  {
    id: 1,
    empresa: "SOLUCIONES TECH S.A.C",
    ruc: "20123456789",
    planActual: "Ilimitado",
    estadoPago: "al día",
    estadoSuscripcion: "activo",
    proximoVence: "2024-12-31",
    historial: [
      {
        plan: "Ilimitado",
  ...
  {
    id: 2,
    empresa: "MINERA DEL SUR",
    ruc: "20987654321",
    planActual: "Premium",
    estadoPago: "vencido",
    estadoSuscripcion: "suspendido",
    proximoVence: "2024-04-15",
    historial: [
      {
        plan: "Premium",
...
const GestionPlanes: React.FC = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<
    "todos" | "activo" | "suspendido" | "cancelado"
  >("todos");
  ...
  const filteredClientes = useMemo(() => {
    return clientes.filter((cliente) => {
      ...
      const matchesFilter =
        filterStatus === "todos" || cliente.estadoSuscripcion === filterStatus;
      return matchesSearch && matchesFilter;
    });
  }, [searchTerm, filterStatus, clientes]);

  const handleConfirmCambio = (data: {
    nuevoPlan: string;
    monto: string;
    fechaInicio: string;
  }) => {
    ...
    console.log(
      `Plan de ${selectedCliente?.empresa} actualizado a ${data.nuevoPlan}`,
    );
  };
  if (loading) {
    return (
      <LoadingSpinner
        message="Cargando gestión de planes..."
        fullScreen={true}
        size="md"
      />
    );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-1">
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium ">
            Gestión de facturación y ciclos de vida
          </p>
        </div>
        ...
        <div className="xl:col-span-7 space-y-6">
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(["todos", "activo", "suspendido", "cancelado"] as const).map(
                (status) => (
                  <button
                    key={status}
              ...
              <table className="w-full border-separate border-spacing-0">
                <thead>
                  <tr className="bg-gray-50/50 dark:bg-gray-900/50 text-gray-400 dark:text-gray-500">
                    ...
                    <th className="px-6 py-5 text-center text-[10px] font-black uppercase tracking-widest">
                      Suscripción
                    </th>
                    <th className="px-6 py-5 text-right text-[10px] font-black uppercase tracking-widest">
                      Estado Pago
                    </th>
                  </tr>
                </thead>
                    ...
                    >
                      <td className="px-6 py-6">
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-2 rounded-lg ${selectedClienteId === cliente.id ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-400 group-hover:text-blue-500"}`}
                          >
                      ...
                      <td className="px-6 py-6 text-center">
                        <span
                          className={`px-3 py-1 rounded-full border text-[9px] font-black uppercase tracking-tighter ${getStatusStyle(cliente.estadoSuscripcion)}`}
                        >
                      ...
                      <td className="px-6 py-6 text-right">
                        <span
                          className={`px-3 py-1 rounded-full border text-[9px] font-black uppercase tracking-tighter ${getStatusStyle(cliente.estadoPago)}`}
                        >
                          {cliente.estadoPago}
                        </span>
                      </td>
            ...
            <div className="bg-white dark:bg-[#161b22] rounded-[2.5rem] border border-gray-200 dark:border-gray-800 p-8 shadow-sm sticky top-10 space-y-8">
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <div
                    className={`p-3 rounded-2xl border shadow-lg ${getStatusStyle(selectedCliente.estadoSuscripcion)}`}
                  >
                ...
                <div className="grid grid-cols-2 gap-3">
                  {selectedCliente.estadoSuscripcion === "activo" ? (
                    <button
                      onClick={() =>
                        updateClienteStatus(selectedCliente.id, "suspendido")
                      }
                    ...
                    <button
                      onClick={() =>
                        updateClienteStatus(selectedCliente.id, "activo")
                      }
                  ...
                  <button
                    onClick={() =>
                      updateClienteStatus(selectedCliente.id, "cancelado")
                    }
              ...
              <div className="space-y-6 bg-gray-50 dark:bg-gray-900/30 p-6 rounded-[2rem] border border-gray-100 dark:border-gray-800">
                ...
                <div className="space-y-6">
                  {selectedCliente.historial.map((item, idx) => (
                    <div
                      key={idx}
                      className="relative pl-6 border-l-2 border-gray-200 dark:border-gray-800 last:border-0"
                    >
                      <div
                        className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-4 border-white dark:border-[#161b22] shadow-sm ${item.estado === "actual" ? "bg-blue-500" : "bg-gray-300 dark:bg-gray-700"}`}
                      />
                      <div className="space-y-1">
