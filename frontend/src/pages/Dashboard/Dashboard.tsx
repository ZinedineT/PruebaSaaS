...
const mockDataByPeriod = {
  'Hoy': {
    ...
    limites: [
      { cliente: 'Minimarket El Sol', plan: 'Estándar', uso: 2800, limite: 3000 },
      { cliente: 'Botica San José', plan: 'Emprendedor', uso: 1760, limite: 2000 },
      { cliente: 'Comercial Fénix', plan: 'Profesional', uso: 4350, limite: 5000 },
      { cliente: 'Inversiones Rojas', plan: 'Estándar', uso: 2410, limite: 3000 },
    ]
  },
  'Este mes': {
    resumen: { activos: 1284, suspendidos: 96, nuevos: 34, bajas: 12 },
  ...
  'Últimos 30 días': {
    ...
    limites: [
      { cliente: 'Minimarket El Sol', plan: 'Estándar', uso: 2900, limite: 3000 },
      { cliente: 'Botica San José', plan: 'Emprendedor', uso: 1950, limite: 2000 },
      { cliente: 'Comercial Fénix', plan: 'Profesional', uso: 4800, limite: 5000 },
      { cliente: 'Inversiones Rojas', plan: 'Estándar', uso: 2700, limite: 3000 },
    ]
  }
};
...
const blockPermissions: Record<Role, string[]> = {
  super_admin: ['resumen', 'estado_planes', 'cobranza', 'comprobantes', 'alertas', 'limites'],
  administrativo: ['resumen', 'estado_planes', 'cobranza', 'comprobantes', 'alertas', 'limites'],
...
const getLimitStatus = (porcentaje: number) => {
  if (porcentaje >= 90) return { label: 'Crítico', color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-950/30' };
  if (porcentaje >= 75) return { label: 'En alerta', color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-950/30' };
  return { label: 'Observación', color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-950/30' };
};
...
const CardStat = ({ item }: any) => (
  <div className="bg-white dark:bg-[#161b22] p-6 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm">
    <div className="flex justify-between items-start">
      ...
      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Tiempo-Real</span>
    </div>
    <div className="mt-4">
      <h3 className="text-3xl font-black text-gray-900 dark:text-white leading-none">{item.value}</h3>
      <p className="text-[11px] font-bold text-gray-500 uppercase tracking-tight mt-2">{item.name}</p>
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      const now = new Date();
      setLastUpdate(`${now.getHours()}:${now.getMinutes().toString().padStart(2, '0')} ${now.getHours() >= 12 ? 'p. m.' : 'a. m.'}`);
      setLoading(false);
    }, 500);
  }, [periodo]);

  const resumenStats = [
    { name: 'Clientes Activos', value: data.resumen.activos.toLocaleString(), desc: 'Clientes operativos', icon: UsersIcon, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
    { name: 'Suspendidos por pago', value: data.resumen.suspendidos.toLocaleString(), desc: 'Inhabilitados por deuda', icon: ClockIcon, color: 'text-amber-500', bg: 'bg-amber-500/10' },
    { name: 'Nuevos del periodo', value: data.resumen.nuevos.toLocaleString(), desc: 'Altas registradas', icon: UserPlusIcon, color: 'text-blue-500', bg: 'bg-blue-500/10' },
    { name: 'Bajas del periodo', value: data.resumen.bajas.toLocaleString(), desc: 'Salidas del periodo', icon: UserMinusIcon, color: 'text-rose-500', bg: 'bg-rose-500/10' },
  ];
    ...
    <div className="max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8 space-y-8 bg-gray-50/50 dark:bg-[#0f1115] min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-gray-200 dark:border-gray-800 pb-8">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-xl">
            ...
            <span className="text-[10px] font-black text-gray-500 uppercase">Actualizado {lastUpdate}</span>
          </div>
        </div>
      </div>
      ...
      {hasBlock('resumen') && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {resumenStats.map((stat) => <CardStat key={stat.name} item={stat} />)}
        </div>
      )}
              ...
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  <tr className='text-gray-900 dark:text-white'>
                    ...
                    <td className="py-3 text-right font-mono">{planesDistribucion.find(p => p.name === 'Emprendedor')?.activos}</td>
                    <td className="py-3 text-right font-mono">{planesDistribucion.find(p => p.name === 'Estándar')?.activos}</td>
                    <td className="py-3 text-right font-mono">{planesDistribucion.find(p => p.name === 'Profesional')?.activos}</td>
                  </tr>
                  <tr>
                    <td className="py-3 font-bold text-gray-900 dark:text-white">Suspendidos pago</td>
                    <td className="py-3 text-right font-mono text-rose-500">{planesDistribucion.find(p => p.name === 'Emprendedor')?.suspendidos}</td>
                    <td className="py-3 text-right font-mono text-rose-500">{planesDistribucion.find(p => p.name === 'Estándar')?.suspendidos}</td>
                    <td className="py-3 text-right font-mono text-rose-500">{planesDistribucion.find(p => p.name === 'Profesional')?.suspendidos}</td>
                  </tr>
                  <tr className="border-t border-gray-200 text-gray-900 dark:text-white dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                    <td className="py-3 font-black text-gray-900 dark:text-white">Total</td>
                    <td className="py-3 text-right font-black font-mono">
                      {(planesDistribucion.find(p => p.name === 'Emprendedor')?.activos || 0) + 
                      (planesDistribucion.find(p => p.name === 'Emprendedor')?.suspendidos || 0)}
                    </td>
                    <td className="py-3 text-right font-black font-mono">
                      {(planesDistribucion.find(p => p.name === 'Estándar')?.activos || 0) + 
                      (planesDistribucion.find(p => p.name === 'Estándar')?.suspendidos || 0)}
                    </td>
                    <td className="py-3 text-right font-black font-mono">
                      {(planesDistribucion.find(p => p.name === 'Profesional')?.activos || 0) + 
                      (planesDistribucion.find(p => p.name === 'Profesional')?.suspendidos || 0)}
                    </td>
                  </tr>
                </tbody>
              </table>
                ...
                return (
                  <div key={plan.name} className="space-y-1">
                    <div className="flex justify-between text-[11px] font-black uppercase">
                      ...
                      <span className="text-gray-400">{plan.activos} / <span className="text-rose-500">{plan.suspendidos}</span></span>
                    </div>
              ...
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {data.cobranza.map((row) => {
                    ...
                    return (
                      <tr key={row.plan} className="text-gray-900 dark:text-white">
                        <td className="py-3 font-bold text-gray-900 dark:text-white">{row.plan}</td>
                        <td className="py-3 text-right font-bold">S/ {row.total.toLocaleString()}</td>
                        <td className="py-3 text-right font-bold text-emerald-600 dark:text-emerald-400">S/ {row.pagado.toLocaleString()}</td>
                        <td className="py-3 text-right font-bold text-rose-500">S/ {row.debe.toLocaleString()}</td>
                        <td className="py-3 text-right font-bold text-gray-500">
                          <div className="flex items-center gap-2 justify-end">
                            ...
                            <span className="text-[9px] font-bold text-gray-500 dark:text-gray-400">{Math.round(avance)}%</span>
                          </div>
                        </td>
                       </tr>
                    );
                ...
                <tfoot className="border-t border-gray-200 dark:border-gray-700">
                  <tr className="font-bold text-gray-900 dark:text-white">
                    ...
                    <td className="pt-3 text-right">S/ {totalGeneral.toLocaleString()}</td>
                    <td className="pt-3 text-right text-emerald-600 dark:text-emerald-400">S/ {totalPagado.toLocaleString()}</td>
                    <td className="pt-3 text-right text-rose-500 dark:text-rose-400">S/ {(totalGeneral - totalPagado).toLocaleString()}</td>
                    <td className="pt-3 text-right font-bold text-gray-500">
                      <div className="flex items-center gap-2 justify-end">
                        <div className="w-16 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                          <div className="h-full bg-blue-500 rounded-full" style={{ width: `${avancePorcentaje}%` }} />
                        </div>
                        <span className="text-[9px] font-bold text-blue-600">{Math.round(avancePorcentaje)}%</span>
                      </div>
                    </td>
                   </tr>
                </tfoot>
        ...
        {hasBlock('comprobantes') && (
          <div className="bg-white dark:bg-[#161b22] rounded-[2.5rem] p-8 border border-gray-100 dark:border-gray-800">
            ...
            <div className="grid grid-cols-3 gap-6">
              {[
                { label: 'Pend. envío', value: 57, status: 'Revisión', color: 'text-blue-500', bg: 'bg-blue-50', icon: ShieldCheckIcon },
                { label: 'Rechazados', value: 23, status: 'Crítico', color: 'text-rose-500', bg: 'bg-rose-50', icon: XCircleIcon },
                { label: 'Pend. anulación', value: 11, status: 'Seguimiento', color: 'text-amber-500', bg: 'bg-amber-50', icon: ExclamationTriangleIcon },
              ].map((item) => (
                <div key={item.label} className="p-6 rounded-3xl bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800">
                  <item.icon className={`w-5 h-5 ${item.color} mb-4`} />
                  <p className="text-3xl font-black dark:text-white">{item.value}</p>
                  <p className="text-[10px] font-black uppercase text-gray-400 mt-1">{item.label}</p>
                  <span className={`inline-block mt-3 px-3 py-1 rounded-full text-[8px] font-black uppercase ${item.color} ${item.bg} dark:bg-opacity-10`}>
                    {item.status}
                  </span>
                </div>
              ))}
            ...
            <div className="space-y-4">
              <div className="bg-white/10 backdrop-blur-md p-5 rounded-2xl border border-white/10">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><p className="text-2xl font-black">{data.vencimientos.clientes7}</p><p className="text-[8px] font-bold uppercase text-blue-200">En 7 días</p></div>
                  <div><p className="text-2xl font-black">{data.vencimientos.clientes30}</p><p className="text-[8px] font-bold uppercase text-blue-200">En 30 días</p></div>
                </div>
              </div>
              ...
              <div className="bg-white/10 backdrop-blur-md p-5 rounded-2xl border border-white/10">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-[11px] font-black uppercase">CDT por vencer</span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div><p className="text-2xl font-black">{data.vencimientos.cdt7}</p><p className="text-[8px] font-bold uppercase text-blue-200">En 7 días</p></div>
                  <div><p className="text-2xl font-black">{data.vencimientos.cdt30}</p><p className="text-[8px] font-bold uppercase text-blue-200">En 30 días</p></div>
                </div>
              </div>
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {data.limites.map((item) => {
                  ...
                  const barColor = porcentaje >= 90 ? 'bg-rose-500' : porcentaje >= 75 ? 'bg-amber-500' : 'bg-blue-500';
                  return (
                    <tr key={item.cliente} className="hover:bg-gray-50 dark:hover:bg-gray-900/40 transition-colors">
      ...
      <div className="text-center text-[8px] text-gray-400 uppercase tracking-wider border-t border-gray-200 dark:border-gray-800 pt-6 mt-4">
        Modo: {userRole.replace('_', ' ')} • Dashboard preparado para permisos por rol
      </div>
    </div>
  );
};
